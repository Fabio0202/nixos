...
require("lazy").setup({
	-- Omarchy theme integration (lua/plugins/*: theme, all-themes, omarchy-theme)
	{ import = "plugins" },
	...
	-- Indent guides: only the current indent scope (mini.indentscope).
	-- Static all-level guides from indent-blankline are disabled on purpose.
	{
	...
	-- Active scope indicator (static line on the current indent block)
	{
	...
	-- Treesitter parsers. Upstream on NixOS these came precompiled from the
	-- flake; on this Arch box lazy installs the plugin and compiles parsers.
	{
		...
		config = function()
			local wanted = {
				"lua", "vim", "vimdoc", "bash", "c", "cpp", "cmake",
